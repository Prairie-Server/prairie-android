...
import org.prairieserver.prairie.model.settings.SettingScope
import org.prairieserver.prairie.model.settings.SettingScopeIdentity
import org.prairieserver.prairie.model.settings.SettingValueWriteRequest
...
import org.prairieserver.prairie.model.settings.UpdateSettingRequest
...
open class SettingsApi(private val client: HttpClient) {
    ...
    open suspend fun putValue(
        ...
        client.put("/api/v1/settings/values/$key") {
            ...
            if (mutationId.isNotBlank()) {
                header("X-Prairie-Mutation-Id", mutationId)
            }
            contentType(ContentType.Application.Json)
     ...
     * `X-Prairie-Device-Id`, and appending it again would send two values.
     */
