...
/** Attribute carrying the [AuthScopeSnapshot] that [PrairieAuthPlugin] honors. */
val AuthScopeAttributeKey: AttributeKey<AuthScopeSnapshot> = AttributeKey("PrairieAuthScope")
 ...
 */
val SkipPrairieAuthAttributeKey: AttributeKey<Boolean> = AttributeKey("PrairieSkipAuth")

/**
 * Marks a same-origin request as unsafe to send without a Prairie bearer.
 *
 * This is intentionally opt-in: public endpoints and [skipPrairieAuth] requests
 * retain their existing behavior.
 */
internal val RequirePrairieAuthAttributeKey: AttributeKey<Boolean> = AttributeKey("PrairieRequireAuth")

/** Pin this request to [snapshot]'s scope; [PrairieAuthPlugin] uses it verbatim. */
...
internal fun HttpRequestBuilder.requirePrairieAuth() {
    attributes.put(RequirePrairieAuthAttributeKey, true)
}

/** Opt this request out of Prairie auth/profile headers and auth-refresh handling. */
fun HttpRequestBuilder.skipPrairieAuth() {
    attributes.put(SkipPrairieAuthAttributeKey, true)
}
