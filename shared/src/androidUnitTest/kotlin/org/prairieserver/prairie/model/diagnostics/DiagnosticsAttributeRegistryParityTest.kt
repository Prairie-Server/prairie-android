...
class DiagnosticsAttributeRegistryParityTest {
    ...
    fun registryBackedTypeValidationCoversNewlyRegisteredKeys() {
        ...
        assertFailsWith<DiagnosticsValidationException> {
            line.copy(attributes = mapOf("duration_ms" to PrairieJson.encodeToJsonElement("42"))).validate()
        }
        assertFailsWith<DiagnosticsValidationException> {
            line.copy(attributes = mapOf("phase" to PrairieJson.encodeToJsonElement(7))).validate()
        }
    }

    private fun canonicalRegistry(): Map<String, Map<String, String>> {
        val root = PrairieJson.parseToJsonElement(fixture("attr-registry.json")).jsonObject
    ...
    private fun DiagnosticsLogCategory.wireName(): String =
        PrairieJson.encodeToJsonElement(DiagnosticsLogCategory.serializer(), this).jsonPrimitive.content
