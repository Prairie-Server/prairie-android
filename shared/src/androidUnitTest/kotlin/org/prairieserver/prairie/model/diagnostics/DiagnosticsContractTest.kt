...
class DiagnosticsContractTest {
    ...
    fun canonicalValidManifestFixturesDecodeValidateAndRoundTrip() {
        ...
        fixtures.forEach { fixtureName ->
            ...
            val roundTripped = decodeDiagnosticsManifest(PrairieJson.encodeToString(manifest))
    ...
    fun statusAndUploadResponsesMatchServerWireNames() {
        val status = PrairieJson.decodeFromString<DiagnosticsStatusResponse>(
            """{
                "status":"available",
        ...
        val upload = PrairieJson.decodeFromString<DiagnosticsUploadResponse>(
    ...
    fun profileIdRemainsOptionalForPreProfileManualCapture() {
        ...
        assertFalse(PrairieJson.encodeToString(manifest).contains("profile_id"))
    }
