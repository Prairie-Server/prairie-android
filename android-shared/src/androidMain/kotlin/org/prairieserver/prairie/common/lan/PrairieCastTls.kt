package org.prairieserver.prairie.common.lan
...
/**
 * TLS-PSK for the `_prairiecast._tcp` control channel, matching prairie-apple's
 * `PrairieLANTLS.parameters(psk: "prairie-cast-v1", identity: "prairie-cast")` — the
 ...
 * ([org.prairieserver.prairie.common.pairing.TlsPskPairingTransport]) which is
 ...
 * external-PSK hook.
 */
object PrairieCastTls {
    private val KEY = "prairie-cast-v1".toByteArray(Charsets.UTF_8)
    private val IDENTITY = "prairie-cast".toByteArray(Charsets.UTF_8)
     ...
     */
    fun accept(socket: Socket): PrairieCastTlsSession {
        ...
        try {
            ...
            while (!protocol.isConnected) {
                ...
                if (read < 0) throw EOFException("PrairieCast TLS handshake closed before completion")
                if (read == 0) continue
        ...
        return PrairieCastTlsSession(socket, protocol, rawIn, rawOut)
    }
     ...
     * module.
     */
    fun connect(host: String, port: Int, connectTimeoutMs: Int): PrairieCastTlsClientSession {
        val socket = Socket()
        ...
        return PrairieCastTlsClientSession(socket, protocol)
    }
...
class PrairieCastTlsClientSession internal constructor(
