...
class DefaultDiagnosticsUploader(
    ...
    private sealed interface HostedCreateAttempt {
        ...
        data class Sent(
            val result: HostedDiagnosticsApiResult<org.prairieserver.prairie.network.api.HostedDiagnosticsCreateReportResponse>,
        ) : HostedCreateAttempt
    }
...
internal fun PendingReport.withCurrentConsent(
    ...
    val manifestMode = if (manifest.report.type == DiagnosticsReportType.MANUAL) {
        org.prairieserver.prairie.model.diagnostics.DiagnosticsConsentMode.MANUAL
    } else if (!hosted && mode == DiagnosticsConsentMode.ALWAYS) {
        org.prairieserver.prairie.model.diagnostics.DiagnosticsConsentMode.ALWAYS
    } else {
        org.prairieserver.prairie.model.diagnostics.DiagnosticsConsentMode.PROMPT
    }
    return copy(
        manifest = manifest.copy(
            report = if (hosted) manifest.report.copy(profileId = null) else manifest.report,
            consent = org.prairieserver.prairie.model.diagnostics.DiagnosticsConsent(manifestMode, noticeVersion),
