...
import org.prairieserver.prairie.pairing.PairingMessageCodec
 ...
 * prairie-apple [PairingSession.tlsParameters] byte-for-byte.
 */
internal object PairingPsk {
    val key: ByteArray = "prairie-companion-pairing-v1".toByteArray(Charsets.UTF_8)
    val identity: ByteArray = "prairie-pairing".toByteArray(Charsets.UTF_8)
 ...
 * matching the iOS receiver exactly (PSK = "prairie-companion-pairing-v1", PSK
 * identity = "prairie-pairing", and the PSK cipher suites Apple offers for the
...
class TlsPskPairingTransport private constructor(
    ...
    companion object {
        ...
        fun accept(socket: Socket): TlsPskPairingTransport {
            ...
            val server = PrairiePskTlsServer(crypto)
            try {
 ...
 */
private class PrairiePskTlsServer(
