...
import org.prairieserver.prairie.network.api.DefaultHostedDiagnosticsApi
import org.prairieserver.prairie.network.api.HostedDiagnosticsApi
import org.prairieserver.prairie.network.api.createHostedDiagnosticsClient
...
val diagnosticsModule = module {
    ...
    single {
        BreadcrumbJournal(androidContext().noBackupFilesDir).also(PrairieLog::installBreadcrumbSink)
    }
    ...
    single<HostedDiagnosticsCapabilitiesStore> {
        ...
        object : HostedDiagnosticsCapabilitiesStore {
            ...
            override suspend fun save(capabilities: org.prairieserver.prairie.network.api.HostedDiagnosticsCapabilities) {
    ...
    single<DiagnosticsRuntimePublisher> {
        DefaultDiagnosticsRuntimePublisher(
            captureSessionIdFactory = { PrairieLog.captureSessionId },
            ledger = get(),
            logBuffer = get(),
            deviceSnapshots = get(),
            deviceSnapshotCache = get(),
...
private fun androidExitReportEnvironment(
    ...
    buildIdentity: PrairieClientBuildIdentity,
    ...
    // CI's build counter, the same value the X-Prairie-Client-Build header and the
