...
object DiagnosticsNetworkLogger : NetworkDiagnosticsObserver {
    override fun completed(method: String, rawPath: String, status: Int, durationMs: Long) {
        PrairieLog.i(
            ...
            attributes = mapOf(
                "method" to PrairieLogAttribute.Text(method.uppercase().take(12)),
                "path" to PrairieLogAttribute.Text(safeDiagnosticsNetworkPath(rawPath)),
                "status" to PrairieLogAttribute.Integer(status.toLong().coerceIn(0, 999)),
                "duration_ms" to PrairieLogAttribute.Integer(durationMs.coerceAtLeast(0)),
            ),
        )
    }
    ...
    override fun authRefresh(state: String) {
        ...
        PrairieLog.i(DiagnosticsLogCategory.NETWORK, "Auth", message)
    }
}
...
object DiagnosticsPlaybackLogger {
    ...
    fun startFailure(code: PlaybackDiagnosticsCode?) = playbackInfo(
        "video start failed",
        code?.let { mapOf("failure_code" to PrairieLogAttribute.Text(it.wireValue)) }.orEmpty(),
    )
    ...
    fun startReady(durationMs: Long) {
        if (DiagnosticsCaptureDetailState.isEnabled()) {
            playbackInfo(
                "video start ready",
                mapOf("startup_ready_ms" to PrairieLogAttribute.Integer(durationMs.coerceAtLeast(0))),
            )
        }
    }
    ...
    fun firstFrame(durationMs: Long) {
        if (DiagnosticsCaptureDetailState.isEnabled()) {
            playbackInfo(
                ...
                mapOf("first_frame_ms" to PrairieLogAttribute.Integer(durationMs.coerceAtLeast(0))),
            )
        }
    }
    ...
    fun videoDecoderInitialized(decoderName: String) = playbackInfo(
        ...
        mapOf("decoder" to PrairieLogAttribute.Text(decoderName)),
    )
    ...
    fun videoFormatChanged(format: String?, width: Int, height: Int, hdrMode: String?) = playbackInfo(
        ...
        buildMap {
            format?.let { put("fmt", PrairieLogAttribute.Text(it)) }
            width.takeIf { it > 0 }?.let { put("width", PrairieLogAttribute.Integer(it.toLong())) }
            height.takeIf { it > 0 }?.let { put("height", PrairieLogAttribute.Integer(it.toLong())) }
            hdrMode?.let { put("hdr_mode", PrairieLogAttribute.Text(it)) }
        },
    )

    fun audioFormatChanged(format: String?) = playbackInfo(
        "audio format changed",
        format?.let { mapOf("fmt" to PrairieLogAttribute.Text(it)) }.orEmpty(),
    )

    fun tracksChanged() = playbackInfo("tracks changed")

    fun droppedFrames(count: Int) = PrairieLog.w(
        DiagnosticsLogCategory.PLAYBACK,
        "Media3Analytics",
        "video frames dropped",
        mapOf("dropped_frames" to PrairieLogAttribute.Integer(count.coerceAtLeast(0).toLong())),
    )

    fun audioUnderrun() = PrairieLog.w(DiagnosticsLogCategory.PLAYBACK, "Media3Analytics", "audio underrun")
    ...
    fun statsSnapshot(snapshot: PlayerStatsSnapshot) = playbackInfo(
        ...
        buildMap {
            snapshot.videoDecoderName?.let { put("decoder", PrairieLogAttribute.Text(it)) }
            snapshot.videoMimeType?.let { put("fmt", PrairieLogAttribute.Text(it)) }
            snapshot.videoWidth?.let { put("width", PrairieLogAttribute.Integer(it.toLong())) }
            snapshot.videoHeight?.let { put("height", PrairieLogAttribute.Integer(it.toLong())) }
            snapshot.hdrMode?.let { put("hdr_mode", PrairieLogAttribute.Text(it)) }
            snapshot.bitrateBps?.let { put("bitrate_kbps", PrairieLogAttribute.Integer((it / 1_000).coerceAtLeast(0))) }
            put("dropped_frames", PrairieLogAttribute.Integer(snapshot.droppedFrames.coerceAtLeast(0).toLong()))
            put("audio_underruns", PrairieLogAttribute.Integer(snapshot.audioUnderruns.coerceAtLeast(0).toLong()))
            snapshot.startupReadyMs?.let { put("startup_ready_ms", PrairieLogAttribute.Integer(it.coerceAtLeast(0))) }
            snapshot.firstFrameMs?.let { put("first_frame_ms", PrairieLogAttribute.Integer(it.coerceAtLeast(0))) }
            snapshot.bufferedDurationMs?.let { put("buffered_ms", PrairieLogAttribute.Integer(it.coerceAtLeast(0))) }
            put("rebuffer_count", PrairieLogAttribute.Integer(snapshot.rebufferCount.coerceAtLeast(0).toLong()))
            put("rebuffer_total_ms", PrairieLogAttribute.Integer(snapshot.rebufferTotalMs.coerceAtLeast(0)))
            put("rebuffer_max_ms", PrairieLogAttribute.Integer(snapshot.rebufferMaxMs.coerceAtLeast(0)))
            put("seek_count", PrairieLogAttribute.Integer(snapshot.seekCount.coerceAtLeast(0).toLong()))
            snapshot.lastSeekDurationMs?.let { put("seek_last_ms", PrairieLogAttribute.Integer(it.coerceAtLeast(0))) }
            put("seek_total_ms", PrairieLogAttribute.Integer(snapshot.seekTotalMs.coerceAtLeast(0)))
            put("seek_max_ms", PrairieLogAttribute.Integer(snapshot.seekMaxMs.coerceAtLeast(0)))
        },
    )
    ...
    ) = PrairieLog.i(DiagnosticsLogCategory.PLAYBACK, "Media3Analytics", message, attributes)
}

object DiagnosticsLifecycleLogger {
    fun state(state: String) = PrairieLog.breadcrumb(
        DiagnosticsLogCategory.LIFECYCLE,
        "AppLifecycle",
        "app lifecycle changed",
        mapOf("state" to PrairieLogAttribute.Text(state)),
    )
...
object DiagnosticsPerformanceLogger {
    fun snapshot(
        ...
        buildMap {
            put("route", PrairieLogAttribute.Text(DiagnosticsPerformanceRoute.current()))
            put("frame_count", PrairieLogAttribute.Integer(frames.frameCount.coerceAtLeast(0)))
            put("slow_frame_count", PrairieLogAttribute.Integer(frames.slowFrameCount.coerceAtLeast(0)))
            put("p95_frame_ms", PrairieLogAttribute.Integer(frames.p95FrameMs.coerceAtLeast(0)))
            put("worst_frame_ms", PrairieLogAttribute.Integer(frames.worstFrameMs.coerceAtLeast(0)))
            put("main_stall_count", PrairieLogAttribute.Integer(frames.mainThreadStallCount.coerceAtLeast(0)))
            put("main_stall_max_ms", PrairieLogAttribute.Integer(frames.longestMainThreadStallMs.coerceAtLeast(0)))
            put("java_heap_mb", PrairieLogAttribute.Integer(resources.javaHeapMb.coerceAtLeast(0)))
            put("process_pss_mb", PrairieLogAttribute.Integer(resources.processPssMb.coerceAtLeast(0)))
            put("low_memory", PrairieLogAttribute.Flag(resources.lowMemory))
            resources.thermalStatus?.let { put("thermal_status", PrairieLogAttribute.Integer(it.toLong().coerceAtLeast(0))) }
            startupFirstFrameMs?.let { put("startup_first_frame_ms", PrairieLogAttribute.Integer(it.coerceAtLeast(0))) }
        },
    )
}

object DiagnosticsCastLogger {
    fun event(message: String) = PrairieLog.i(DiagnosticsLogCategory.CAST, "Cast", message)

    fun warning(message: String) = PrairieLog.w(DiagnosticsLogCategory.CAST, "Cast", message)
}

object DiagnosticsDownloadLogger {
    fun event(message: String) = PrairieLog.i(DiagnosticsLogCategory.DOWNLOAD, "DownloadWorker", message)

    fun warning(message: String) = PrairieLog.w(DiagnosticsLogCategory.DOWNLOAD, "DownloadWorker", message)

    fun error(message: String) = PrairieLog.e(DiagnosticsLogCategory.DOWNLOAD, "DownloadWorker", message)
}

object DiagnosticsFocusLogger {
    fun transition(target: String, action: String) = PrairieLog.d(
        DiagnosticsLogCategory.FOCUS,
        "TvShellFocus",
        "focus transition",
        mapOf(
            "target" to PrairieLogAttribute.Text(target),
            "action" to PrairieLogAttribute.Text(action),
        ),
    )
     ...
     */
    fun contentEntryFailed(route: String) = PrairieLog.w(
        DiagnosticsLogCategory.FOCUS,
        "TvShellFocus",
        "content focus entry failed",
        mapOf("route" to PrairieLogAttribute.Text(route)),
    )
}
