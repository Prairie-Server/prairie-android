...
class Media3OnlyRuntimeSourceTest {
    ...
    fun serviceOwnsOneMedia3PlayerWithoutEngineCommands() {
        val source = File("src/androidMain/kotlin/org/prairieserver/prairie/common/player/PrairiePlaybackService.kt").readText()
    ...
    fun selectorAndFactoryAreMedia3Only() {
        val selector = File(
            "src/androidMain/kotlin/org/prairieserver/prairie/common/player/backend/VideoPlaybackBackendSelector.kt",
        ).readText()
        val factory = File(
            "src/androidMain/kotlin/org/prairieserver/prairie/common/player/backend/VideoPlaybackBackendFactory.kt",
        ).readText()
