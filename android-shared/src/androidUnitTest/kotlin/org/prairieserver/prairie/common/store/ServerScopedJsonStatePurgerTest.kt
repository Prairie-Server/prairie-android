package org.prairieserver.prairie.common.store
...
class ServerScopedJsonStatePurgerTest {
    ...
    fun `orphan symlink is removed without touching its target`() {
        val base = Files.createTempDirectory("prairie-json-purge").toFile()
        val external = Files.createTempDirectory("prairie-json-external")
    ...
    fun `orphan symlink to a registered sibling removes only the link`() {
        val base = Files.createTempDirectory("prairie-json-sibling-link").toFile()
        val root = base.toPath().resolve("ebook_state")
    ...
    fun `symlinked store root is rejected without touching its target`() {
        val base = Files.createTempDirectory("prairie-json-root-purge").toFile()
        val external = Files.createTempDirectory("prairie-json-root-external")
        val sentinel = external.resolve("keep.txt")
        Files.write(sentinel, "keep".toByteArray())
    ...
    fun `store root swapped to a symlink after construction is rejected`() {
        val base = Files.createTempDirectory("prairie-json-root-swap").toFile()
        val root = base.toPath().resolve("ebook_state")
        Files.createDirectories(root)
        val purger = ServerScopedJsonStatePurger(base)
        Files.delete(root)
        val external = Files.createTempDirectory("prairie-json-root-swap-external")
        val sentinel = external.resolve("keep.txt")
        Files.write(sentinel, "keep".toByteArray())
    ...
    fun `purge waits for an in-flight writer and rejects stale writes afterward`() {
        val base = Files.createTempDirectory("prairie-json-writer-race").toFile()
