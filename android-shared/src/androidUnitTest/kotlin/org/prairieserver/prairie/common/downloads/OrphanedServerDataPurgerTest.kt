...
import org.prairieserver.prairie.common.data.sync.OutboxOperation
...
import org.prairieserver.prairie.common.store.ServerScopedJsonStatePurger
...
class OrphanedServerDataPurgerTest {
    ...
    private val db = Room.inMemoryDatabaseBuilder(context, PrairieDatabase::class.java)
    ...
    fun `json-only orphan state is discovered while registered state is retained`() = runTest {
        val baseDir = Files.createTempDirectory("prairie-json-orphans").toFile()
