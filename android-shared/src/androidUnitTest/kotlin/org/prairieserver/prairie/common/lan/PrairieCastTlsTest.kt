...
class PrairieCastTlsTest {
    @Test
    fun androidClientAndServerCompletePskHandshake() {
        val listener = ServerSocket(0)
        val accepted = CompletableFuture<PrairieCastTlsSession>()
        thread(isDaemon = true, name = "prairiecast-tls-test-server") {
            runCatching { PrairieCastTls.accept(listener.accept()) }
                .onSuccess(accepted::complete)
                .onFailure(accepted::completeExceptionally)
        }

        val client = PrairieCastTls.connect("127.0.0.1", listener.localPort, 5_000)
