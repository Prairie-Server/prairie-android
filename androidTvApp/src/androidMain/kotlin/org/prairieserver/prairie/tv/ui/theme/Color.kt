...
val PrairieSecondaryText = Color(0xFFEDEDED).copy(alpha = 0.75f)
...
val FocusedContainer = PrairieOnSurface
...
// Skyline top-bar capsule chrome — mirrors `prairieChromeSelectedFill` /
// `prairieChromeSelectedBorder` in iOS `Theme/Colors.swift`. The focused
// capsule inverts to a solid `PrairieOnSurface` fill (white-on-background);
...
val AccentLavender = PrairieOnSurface
...
val PrairieBlueGlow = Color.White.copy(alpha = 0.28f)
val PrairieBlueBorderIdle = Color.White.copy(alpha = 0.16f)
...
val ProgressFill = PrairieOnSurface
...
val PrairieBlue = PrairieOnSurface
val PrairieBlueLight = PrairieOnSurface
