 ...
 * [prairieCardDefaults] (for Cards) — pick the right one for the surface
 * type, not both.
 */
fun Modifier.prairieFocus(
 ...
 * Usage:
 * ```
 * val defaults = prairieCardDefaults(cardShape)
 * Card(
...
@OptIn(ExperimentalTvMaterial3Api::class)
@Composable
fun prairieCardDefaults(
    shape: Shape,
    focusedScale: Float = 1.10f,
): PrairieCardFocus = PrairieCardFocus(
