package org.siloserver.silo.tv.notifications
...
/**
 * TV-side copy of the phone app's `NotificationsForegroundStarter`.
 *
 * `androidApp` and `androidTvApp` are separate application modules, each with
 * its own `Application` + `startKoin`, so the phone starter's call site can't
 * be reused; this is a deliberate duplicate (keep the two in sync). Candidate
 * for a future hoist into `android-shared`.
 *
