 ...
 * `handleDeepLink` (`continuum://` there; this app's scheme is `prairie://`).
 * Routing is by URL host with the content id as the first path segment:
 *
 * - `prairie://downloads`          → the Downloads tab
 * - `prairie://item/<contentId>`   → item detail
 * - `prairie://play/<contentId>`   → the player (immediate playback)
 *
...
internal fun contentDeepLinkRouteOrNull(rawUri: String?): String? {
    val uri = rawUri
        ...
        ?: return null
    if (!uri.scheme.equals("prairie", ignoreCase = true)) return null
