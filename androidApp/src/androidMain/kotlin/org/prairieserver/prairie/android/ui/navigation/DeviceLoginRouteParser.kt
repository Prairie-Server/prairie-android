 ...
 *
 * TV QR sessions may surface either app-scheme links (`prairie://device?...`)
...
internal sealed interface DeviceLoginScope {
    /** An app-scheme link (`prairie://device`): names no server. */
...
private fun URI.isDeviceLoginUri(): Boolean {
    ...
    return when (scheme) {
        "prairie" -> host.equals("device", ignoreCase = true)
