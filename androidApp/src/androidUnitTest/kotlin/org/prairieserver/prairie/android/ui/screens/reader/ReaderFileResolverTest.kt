...
class ReaderFileResolverTest {
    ...
    fun `leaves absolute and local reader urls unchanged`() {
        ...
        assertEquals(
            "file:///storage/emulated/0/Download/Prairie/book.epub",
            resolveReaderRequestUrl("file:///storage/emulated/0/Download/Prairie/book.epub", "https://lib.strm.cafe"),
        )
        assertEquals(
    ...
    fun `classifies local reader urls after normalization`() {
        assertEquals(
            ...
            readerRequestKind(" file:///storage/emulated/0/Download/Prairie/book.epub ", "https://lib.strm.cafe"),
        )
        assertEquals(
    ...
    fun `file reader urls decode encoded local filesystem paths`() {
        val file = File("/storage/emulated/0/Download/Prairie/The #1 Book.epub")
