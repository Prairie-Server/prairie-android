...
class MobileAppleParitySourceTest {
    private val root = "src/androidMain/kotlin/org/prairieserver/prairie/android"
    ...
    fun rootShellMatchesIosTabAndChromeShape() {
        ...
        assertTrue(
            homeScreen.contains("PrairieWordmark("),
            "Home chrome should include the same left-side Prairie wordmark anchor as iOS HomeView.",
        )
        assertTrue(
            topBar.contains("painterResource(id = R.drawable.prairie_wordmark)"),
            "Shared Android mobile top bars should use the real Prairie wordmark asset.",
        )
        assertFalse(
        ...
        assertTrue(
            search.contains("text = \"Search Prairie\""),
            "The empty Search state should use the iOS-style Search Prairie entry point.",
        )
        assertTrue(
            searchBar.contains("text = \"Search Prairie\""),
