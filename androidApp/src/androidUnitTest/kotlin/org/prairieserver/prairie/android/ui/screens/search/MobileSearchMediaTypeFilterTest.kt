package org.prairieserver.prairie.android.ui.screens.search

import org.prairieserver.prairie.model.catalog.BrowseItem
import kotlin.test.Test
import kotlin.test.assertEquals

class MobileSearchMediaTypeFilterTest {
    @Test
    fun filtersMatchMobileMediaModeTaxonomy() {
        val items = listOf(
            browseItem("a1", "audiobook"),
            browseItem("a2", "audiobooks"),
            browseItem("au1", "audio"),
            browseItem("mu1", "music"),
            browseItem("e1", "ebook"),
            browseItem("e2", "ebooks"),
            browseItem("b1", "book"),
            browseItem("b2", "books"),
            browseItem("c1", "comic"),
            browseItem("c2", "comics"),
            browseItem("manga1", "manga"),
            browseItem("r1", "reading"),
            browseItem("m1", "movie"),
            browseItem("m2", "movies"),
            browseItem("s1", "series"),
            browseItem("sh1", "show"),
            browseItem("tv1", "tv"),
            browseItem("v1", "video"),
        )

        assertEquals(
            listOf("m1", "m2", "s1", "sh1", "tv1", "v1"),
            MobileSearchMediaType.Video.filterResults(items).map { it.contentId },
        )
        assertEquals(
            listOf("a1", "a2", "au1", "mu1"),
            MobileSearchMediaType.Audio.filterResults(items).map { it.contentId },
        )
        assertEquals(
            listOf("e1", "e2", "b1", "b2", "c1", "c2", "manga1", "r1"),
            MobileSearchMediaType.Reading.filterResults(items).map { it.contentId },
        )
    }

    @Test
    fun allFilterPassesItemsThrough() {
        val items = listOf(browseItem("m1", "movie"), browseItem("e1", "ebook"))

        assertEquals(items, MobileSearchMediaType.All.filterResults(items))
    }

    private fun browseItem(contentId: String, type: String): BrowseItem =
        BrowseItem(contentId = contentId, type = type, title = contentId)
}
