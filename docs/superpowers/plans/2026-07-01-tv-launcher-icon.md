...
**Goal:** Replace the Android TV adaptive launcher foreground with a transparent Prairie mark that renders cleanly under Google TV's circular launcher mask.
...
- Produces: Transparent foreground PNGs with centered Prairie mark only.

- [ ] **Step 1: Generate a 432px transparent master foreground**

Use ImageMagick to draw the Prairie mark on a transparent canvas. The mark should fit within the adaptive icon safe zone and leave transparent pixels at all edges.
...
Run: `./gradlew :androidTvApp:testDebugUnitTest --tests org.prairieserver.prairie.tv.ui.theme.TvLauncherIconAssetsTest`
...
Expected: screenshot shows Prairie as a centered mark on the launcher mask, not a cropped rounded-square tile.
