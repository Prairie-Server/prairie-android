...
Make the Prairie tile on sideloaded Fire TV devices look intentional, sharp, and correctly proportioned without changing the Android TV application identity or degrading Google TV launcher behavior.
...
However, the legacy `ic_launcher.png` files are 16:9 banner images. Fire OS 6 on the tested AFTMM device normalizes the application icon into a square area inside its sideloaded-app tile. Feeding that square area a 16:9 source compresses the Prairie artwork and leaves Fire OS's gray framing visually dominant.
...
The icon uses the existing Prairie blue background and white Prairie mark. Artwork remains centered inside a conservative safe area so Fire OS normalization does not crop it. The density files are the fallback for API 24-25 devices, including the tested Fire OS 6 device.
...
Keep `tv_banner.png` as an opaque 320x180 wordmark banner, but compose the complete canonical Prairie logo inside the centered 180x180 square-safe crop. The manifest continues to reference it from both the application and TV activity. This preserves Android TV's 16:9 banner contract while making Fire OS's sideloaded square crop show a complete, legible logo.
...
2. Legacy icons are opaque and preserve the centered Prairie artwork within the safe area.
...
Run the TV unit tests and build a fresh TV APK. Clean-install that APK on the Fire Stick so the launcher cannot reuse the prior package's cached artwork. Capture the Fire TV app-grid screen and confirm that the Prairie artwork is square, undistorted, centered, and legible. Also inspect the Google TV Streamer launcher to ensure the adaptive icon remains correctly masked.

## Future Amazon Appstore Work

If Prairie is later distributed through the Amazon Appstore, provide Amazon's required 1280x720 opaque listing artwork using the same 16:9 wordmark composition. That store-managed asset is the supported path to a full-bleed Fire TV tile.
